...
def gemm_fp8_ptpc_hti_gfx950_kernel(
    ...
    def store_half_tile_to_lds(m_part, n_part, frag_C):
        ...
        for i in range_constexpr(fx.size(frag_C.shape).unpack()):
            row = fx.get_scalar(thr_mma_cRow[i])
            col = fx.get_scalar(thr_mma_cCol[i])
            sC[row, col] = apply_fp8_ptpc_epilogue(
                frag_C[i],
                scale_a_buf,
                scale_b_buf,
                bias_buf if const_expr(param.has_bias) else scale_b_buf,
                block_m_offset + m_part * half_block_m + row,
                block_n_offset + n_part * half_block_n + col,
                m,
                n,
                param.has_bias,
                is_split_k,
...
def gemm_fp8_ptpc(
    ...
    A and B are ``float8_e4m3fn``. Per-token scales are applied in the
    epilogue. Hardware MFMA scale is left at identity.
