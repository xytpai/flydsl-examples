...
def compile_hgemm_kernel(
    ...
    def hgemm_kernel(
        ...
        for bki in range_constexpr(BLOCK_K_LOOPS):
            ...
            gpu.barrier()
            cond = arith.cmpi(arith.CmpIPredicate.eq, tid, fx.Int32(0))
            if cond:
                for mi in range(BLOCK_M):
                    for ni in range(BLOCK_N):
                        acc = c_zero_f
                        for ki in range_constexpr(BLOCK_K):
                            a_data = as_[fx.Index(0), mi, ki]
                            a_data = a_data.extf(T.f32)
                            b_data = bs_[fx.Index(0), ni, ki]
                            b_data = b_data.extf(T.f32)
                            acc = acc + a_data * b_data
                        acc = acc.truncf(dtype_)
                        old = C_[m_offset + mi, n_offset + ni]
                        C_[m_offset + mi, n_offset + ni] = old + acc
